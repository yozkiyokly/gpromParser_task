...
# взлом этого паттерна возможен применением специально созданного домена _первого_ уровня вида .hack-zz 
# и присыланием письма с h@acke-co.hac-me. \b.{6}-.{6}-.{2}\b
# ради темпа обработки не делаю проверку на маловерояное несодержание @, 
# но так и быть, ограничиваю набор знаков через \w     
#           \b\w{6}-\w{6}-\w{2}\b|  			### безопасности для:
...
# Очередь вытрясти флаг, в неизменном виде. Дальше его статус будет являться признаком участия записи в поиске.
# Строка лога может (!) содержать более одного флага, поэтому пришлось нашинковать красивый длинный регексп на малые.
############
...
# Дело в том, что лог содержит пустые отчёты о подключениях к SMTP, полезных данных такие строки будто бы не содержат
# стало быть и пихать их в БД незачем, однако отметим и этот факт цитатой из лога: 
##########################
